...
|-------------------------------------------------|----------------------------------------------------|---------|
| `GET /api/ai/conversations`                     | List the caller's own summaries (newest first)     | `{ success, conversations: Summary[] }` |
| `POST /api/ai/conversations`                    | Create a conversation. Optional `title`; optional `messages[]` **only** for the explicit "save guest conversation to my account" flow (messages copied verbatim). | `201 { success, conversation }` |
| `GET /api/ai/conversations/:id`                 | Full conversation incl. messages (owner only)      | `{ success, conversation }` |
| `PUT /api/ai/conversations/:id`                 | Rename: `{ title }` (1–80 chars)                   | `{ success, conversation: Summary }` |
| `DELETE /api/ai/conversations/:id`              | Delete one (owner only)                            | `{ success, deleted: true }` |
| `DELETE /api/ai/conversations`                  | Delete **all** of the caller's conversations       | `{ success, deleted: true }` |
| `POST /api/ai/conversations/:id/messages`       | Append: `{ role, content }`; auto-titles the conversation from the first user message | `201 { success, message }` |
