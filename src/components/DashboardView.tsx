...
export const DashboardView: React.FC<DashboardViewProps> = ({
  ...
  const handleShareSubmit = (e: React.FormEvent) => {
    ...
    grantConsent({
      ...
      scope: shareScopes,
      expiresDate: new Date(Date.now() + shareDurationDays * 86400000).toISOString().split('T')[0]
    });
    const fakeToken = `GH-AUTH-${Math.random().toString(36).substr(2, 9).toUpperCase()}`;
    setCreatedShareToken(fakeToken);
  };
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs">
          <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl border border-slate-200 animate-in fade-in zoom-in-95">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => { setIsShareModalOpen(false); setCreatedShareToken(null); }} className="text-slate-400 hover:text-slate-600">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            {!createdShareToken ? (
              ...
              <div className="space-y-4 text-center py-2">
                ...
                <div className="p-3 bg-slate-100 rounded-xl font-mono text-xs text-slate-800 break-all select-all">
                  https://globalhealth.med/auth/verify?token={createdShareToken}
                </div>
                <button
                  onClick={() => {
                    navigator.clipboard.writeText(`https://globalhealth.med/auth/verify?token=${createdShareToken}`);
